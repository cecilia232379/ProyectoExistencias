...
(function ($) {
	...
	$.addFlex = function (t, p) {
		...
		p = $.extend({ //apply default properties
			...
			title: false,
			idProperty: 'id',
			pagestat: 'Mostrando {from} hasta {to} de {total} items',
			pagetext: 'Página',
			outof: 'de',
			findtext: 'Buscar',
		...
		if (p.usepager) {
			...
			if (p.searchitems) {
				...
				$(g.sDiv).append("<div class='sDiv2'>" + p.findtext +
						...
						" <select name='qtype'>" + sopt + "</select><input class='qsbox' id='BotonBuscar' type='button' name='ValButon' id = 'ValButon' value='Buscar'></div>");
				...
				$('select[name=qtype]', g.sDiv).keydown(function (e) {
					if (e.keyCode == 13) {
						g.doSearch();
                                ...
                                $('input[name=ValButon]', g.sDiv).click(function () {
					g.doSearch();
				});
